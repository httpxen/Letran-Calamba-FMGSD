...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $video_destination = $lesson['video_url'] ?? '';
  if (isset($_FILES['video']) && $_FILES['video']['error'] === UPLOAD_ERR_OK && is_uploaded_file($_FILES['video']['tmp_name'])) {
    ...
    if (!in_array($file_type, $allowed_types)) {
      ...
      $video_destination = $uploads_dir . $unique_name;

      $ffmpeg_cmd = "ffmpeg -i " . escapeshellarg($file_tmp) . " -vcodec libx264 -acodec aac -strict -2 " . escapeshellarg($video_destination) . " 2>&1";
  ...
  if (empty($errors)) {
    try {
      ...
      if (isset($file_tmp) && file_exists($video_destination)) {
        // Delete old video if it exists and a new one is uploaded
        if ($lesson['video_url'] && file_exists($lesson['video_url']) && $lesson['video_url'] !== $video_destination) {
      ...
      // Refresh lesson data to reflect updates
      $lessonStmt->execute([':lesson_id' => $lesson_id]);
      $lesson = $lessonStmt->fetch(PDO::FETCH_ASSOC);
...
<body class="bg-gray-50 text-gray-900 font-sans antialiased">
      ...
      <main class="flex-1 p-6 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 bg-white p-8 rounded-xl shadow-soft">
            <form id="lesson-form" class="space-y-6" method="POST" enctype="multipart/form-data">
              <?php if (isset($_SESSION['success_message'])) : ?>
                <p class="text-green-500 text-sm font-medium bg-green-50 p-3 rounded-lg animate-fade-in"><?php echo htmlspecialchars($_SESSION['success_message']); ?></p>
                <?php unset($_SESSION['success_message']); ?>
              <?php endif; ?>
              <?php if (isset($errors['database'])) : ?>
                <p class="text-red-500 text-sm font-medium bg-red-50 p-3 rounded-lg"><?php echo htmlspecialchars($errors['database']); ?></p>
              <?php endif; ?>
              <?php if (isset($errors['video'])) : ?>
                <p class="text-red-500 text-sm font-medium bg-red-50 p-3 rounded-lg"><?php echo htmlspecialchars($errors['video']); ?></p>
              <?php endif; ?>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  ...
                  <?php if (isset($errors['title'])) : ?>
                    <p class="text-red-500 text-xs font-medium mt-1"><?php echo htmlspecialchars($errors['title']); ?></p>
                  <?php endif; ?>
                </div>
                <div class="space-y-2">
                  <label for="video" class="block text-sm font-medium text-gray-700">Upload New Video (MP4/WEBM/OGG)</label>
                  <input type="file" id="video" name="video" class="w-full p-3 border border-gray-200 rounded-lg text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-primary-50 file:text-primary-600 hover:file:bg-primary-100 transition-all duration-200" accept="video/mp4,video/webm,video/ogg">
                  <?php if (isset($errors['video'])) : ?>
                    <p class="text-red-500 text-xs font-medium mt-1"><?php echo htmlspecialchars($errors['video']); ?></p>
                  <?php endif; ?>
                </div>
              </div>
              ...
              <div class="flex justify-end space-x-4">
                ...
                <button type="submit" id="save-lesson-btn" class="px-6 py-3 bg-gradient-to-r from-primary-600 to-primary-700 text-white rounded-lg hover:from-primary-700 hover:to-primary-800 font-medium transition-all duration-200 ease-in-out transform hover:scale-105">Save</button>
              </div>
            </form>
          </div>
  ...
  <div id="confirmation-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 animate-fade-in">
    <div class="bg-white rounded-2xl shadow-glow p-8 max-w-md w-full animate-scale-in">
      ...
      <p class="text-gray-600 mb-8">Are you sure you want to update this lesson? This action will save all changes.</p>
      <div class="flex justify-end space-x-4">
  ...
  <script>
    ...
    form.addEventListener('submit', (e) => {
      ...
      const existingVideo = '<?php echo $lesson['video_url'] ? true : false; ?>';

      document.querySelectorAll('.error-message').forEach(el => el.remove());

      if (!title) {
        hasError = true;
        addErrorMessage('title', 'Video title is required');
      }

      if (!video && !existingVideo) {
        hasError = true;
        addErrorMessage('video', 'Video is required');
      } else if (video && !['video/mp4', 'video/webm', 'video/ogg', 'video/x-webm', 'video/x-matroska'].includes(video.type)) {
        hasError = true;
        addErrorMessage('video', 'Only MP4, WEBM, and OGG files are allowed');
      }

      if (hasError) {
        saveLessonBtn.textContent = 'Save';
    ...
    function addErrorMessage(fieldId, message) {
      ...
      error.className = 'text-red-500 text-xs font-medium mt-1 error-message';
