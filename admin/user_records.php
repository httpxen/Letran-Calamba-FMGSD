...
if (!empty($filter_lesson)) {
    $sql .= " AND l.title = :filter_lesson";
    $params[':filter_lesson'] = $filter_lesson;
}

if (!empty($filter_status)) {
    $sql .= " AND qr.isPassed = :filter_status";
    $params[':filter_status'] = $filter_status === 'passed' ? 1 : 0;
}

if (!empty($filter_date_from)) {
    $sql .= " AND qr.taken_at >= :filter_date_from";
    $params[':filter_date_from'] = $filter_date_from;
}

if (!empty($filter_date_to)) {
    $sql .= " AND qr.taken_at <= :filter_date_to";
    $params[':filter_date_to'] = $filter_date_to . ' 23:59:59';
}
...
<body class="bg-gray-50 text-gray-900 font-sans antialiased">
    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r shadow-lg transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out md:static md:shadow-none">
            ...
            <nav class="mt-6">
                <ul class="space-y-1 px-4">
                    ...
                    <li>
                        <a href="user_records.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary-50 text-primary-600 font-medium transition-colors duration-200">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <li>
                        <a href="module_list.php" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-700 hover:bg-primary-50 hover:text-primary-600 font-medium transition-colors duration-200">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <header class="bg-white shadow-sm flex justify-between items-center px-6 py-4">
                ...
                <div class="flex items-center space-x-4">
                    <div class="relative w-64">
                        <input type="text" id="search" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search by user name..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-600 bg-white shadow-sm">
                        ...
                        <div>
                            ...
                            <select id="filter_lesson" name="filter_lesson" class="mt-1 block w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-600 bg-white">
                                ...
                                <?php foreach ($lessonTitles as $lesson): ?>
                                    <option value="<?= htmlspecialchars($lesson) ?>" <?= $filter_lesson === $lesson ? 'selected' : '' ?>>
                    ...
                    <div class="mt-4 flex justify-end gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors duration-200 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 0 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-7.5 0h7.5m-12-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" />
                            </svg>
                            Apply
                        </button>
                    </div>
