...
$check_last_updated = $conn->query("SHOW COLUMNS FROM users LIKE 'last_updated'");
if ($check_last_updated->num_rows == 0) {
    $conn->query("ALTER TABLE users ADD COLUMN last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
}

// Get current user info
$user_id = $_SESSION['user_id'];
$query = "SELECT fullname, email, profile_picture, password, last_updated FROM users WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
                ...
                if ($_FILES["profile_picture"]["size"] <= 5000000) {
                    if (in_array($imageFileType, ['jpg', 'jpeg', 'png', 'gif'])) {
                        if (move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file)) {
                            if (!empty($user['profile_picture']) && $user['profile_picture'] != '../assets/images/profile-placeholder.png' && file_exists($user['profile_picture'])) {
        ...
        if (!empty($update_fields) && !isset($error_message)) {
            $update_query = "UPDATE users SET " . implode(", ", $update_fields) . ", last_updated = NOW() WHERE id = ?";
...
<head>
    ...
    <title>SuperAdmin Account Settings</title>
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-white border-r shadow-soft transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out z-50 md:static md:shadow-none flex flex-col">
            ...
            <nav class="mt-6 flex-1">
                <ul class="space-y-1 px-4">
                    ...
                    <li>
                        <a href="user_approvals.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-primary-50 hover:text-primary-600 font-medium transition-all duration-200 ease-in-out transform hover:scale-[1.02]">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <li>
                        <a href="user_records.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-primary-50 hover:text-primary-600 font-medium transition-all duration-200 ease-in-out transform hover:scale-[1.02]">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.158.433-.158.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                            </svg>
                            User Records
                        </a>
                    </li>
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="bg-white shadow-soft flex justify-between items-center px-6 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-2xl font-semibold text-dashboard">Account Settings</h1>
                </div>
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        <span class="text-gray-600 font-medium hidden sm:block"><?php echo htmlspecialchars($user['fullname']); ?></span>
                        <img src="<?php echo htmlspecialchars($profile_pic); ?>" class="w-10 h-10 rounded-full border border-gray-200 shadow-sm" alt="Profile Picture" />
                    </div>
                </div>
            </header>
                        ...
                        <div class="bg-white p-8 rounded-xl shadow-soft animate-fade-in">
                            ...
                            <form method="POST" enctype="multipart/form-data" class="space-y-6">
                                ...
                                <div class="flex items-center space-x-6 mb-6">
                                    <div class="relative">
                                        <img id="profile-preview" src="<?php echo htmlspecialchars($profile_pic); ?>" class="w-24 h-24 rounded-full border border-gray-200 shadow-sm object-cover" alt="Profile Picture" />
                                        <label for="profile_picture" class="absolute bottom-0 right-0 bg-primary-600 text-white rounded-full p-2 cursor-pointer hover:bg-primary-700 transition-all duration-200">
                                            ...
                                            <input type="file" id="profile_picture" name="profile_picture" accept="image/*" class="hidden" onchange="previewProfilePic(event)" />
                                        </label>
                                    </div>
                                    <div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="text" name="fullname" id="fullname" value="<?php echo htmlspecialchars($user['fullname']); ?>" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 transition-all duration-200" required />
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="email" name="email" id="email" value="<?php echo htmlspecialchars($user['email']); ?>" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 transition-all duration-200" required />
                                        <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                ...
                                <div class="flex justify-end">
                                    <button type="submit" class="px-6 py-3 bg-gradient-to-r from-primary-600 to-primary-700 text-white rounded-lg hover:from-primary-700 hover:to-primary-800 font-medium transition-all duration-200 ease-in-out transform hover:scale-105">
                                        Save Profile
                                    </button>
                                </div>
                            </form>
                        ...
                        <div class="bg-white p-8 rounded-xl shadow-soft animate-fade-in">
                            ...
                            <form method="POST" class="space-y-6">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="current_password" id="current_password" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 transition-all duration-200" />
                                        <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="new_password" id="new_password" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 transition-all duration-200" oninput="checkPasswordStrength(this.value)" />
                                        <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                                        </svg>
                                    </div>
                                    <div id="password-strength" class="text-xs mt-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="confirm_password" id="confirm_password" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 transition-all duration-200" />
                                        <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                                        </svg>
                                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-soft animate-fade-in">
                            ...
                            <div class="flex items-center space-x-4 mb-4">
                                <img src="<?php echo htmlspecialchars($profile_pic); ?>" class="w-16 h-16 rounded-full border border-gray-200 shadow-sm" alt="Profile Picture" />
                                <div>
                            ...
                            <div class="space-y-3">
                                <p class="text-sm text-gray-600"><span class="font-medium">Role:</span> SuperAdmin</p>
                                <p class="text-sm text-gray-600"><span class="font-medium">Last Updated:</span> <?php echo $user['last_updated'] ? date('M d, Y H:i', strtotime($user['last_updated'])) : 'N/A'; ?></p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-soft animate-fade-in">
                            ...
                            <div class="space-y-3">
                                <a href="dashboard.php" class="flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary-600 rounded-lg hover:bg-primary-100 font-medium transition-all duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />
                                    </svg>
                                    Back to Dashboard
                                </a>
                                <a href="../logout.php" class="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 font-medium transition-all duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0l3-3m0 0-3-3m3 3H9" />
                                    </svg>
                                    Logout
